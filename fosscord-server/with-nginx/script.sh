...
sed -i 's/placeholder_frontend/'$SERVER_URL'/' /etc/nginx/sites-enabled/fosscord.conf
certbot --nginx --agree-tos -m $MAIL_CERTBOT --domains $NGINX_HOST,$SERVER_URL  --non-interactive --test-cert
